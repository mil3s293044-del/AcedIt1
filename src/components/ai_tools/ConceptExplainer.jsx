...
import { Loader2, Save, FolderOpen, Trash2, Eye, RefreshCw, Brain, Zap, ChevronDown, ExternalLink } from 'lucide-react';
...
export default function ConceptExplainer() {
    ...
    const handleGenerate = async () => {
        ...
        const response = await base44.integrations.Core.InvokeLLM({
            prompt: `You are an expert ${subject} tutor. Explain the concept: "${concept}".
...
Make it genuinely educational and accurate for VCE Year 12 level.`,
    ...
    const handleGenerateQuiz = async () => {
        ...
        const response = await base44.integrations.Core.InvokeLLM({
            prompt: `Based on this explanation of "${concept}" in ${subject}, create 5 multiple choice questions to test understanding.\n\n${result}`,
            response_json_schema: {
                type: 'object',
                properties: {
                    questions: {
    ...
    return (
        <div className="space-y-5 max-w-3xl">
            {isGenerating && <AILoadingProgress stage="generating" message="Explaining the concept..." estimatedTime={20} />}

            {/* Input Panel */}
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="bg-gradient-to-r from-emerald-500 to-teal-600 px-5 py-4">
                    <h2 className="text-white font-bold text-lg">Concept Explainer</h2>
                    <p className="text-white/70 text-sm">Get any concept explained clearly, at the right depth for you</p>
                </div>
                <div className="p-5 space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                            <label className="text-xs font-bold text-gray-500 uppercase tracking-wide">Concept or Topic</label>
                            <Input
                    ...
                    {/* Depth selector */}
                    <div className="space-y-1.5">
                        <label className="text-xs font-bold text-gray-500 uppercase tracking-wide">Explanation Depth</label>
                    ...
                    {/* Style selector */}
                    <div className="space-y-1.5">
                        <label className="text-xs font-bold text-gray-500 uppercase tracking-wide">Explanation Style</label>
                    ...
                    <Button
                        ...
                        disabled={isGenerating || !concept.trim() || !subject}
                        className="w-full bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 h-11 font-semibold shadow-lg"
                    >
                        {isGenerating ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" />Explaining...</> : <><Brain className="w-4 h-4 mr-2" />Explain This Concept</>}
                    </Button>
                </div>
            </div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                        <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-2xl p-4 flex items-center gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-bold text-gray-900">{concept}</p>
                                <p className="text-xs text-gray-500">{subject} · Explanation ready</p>
                            </div>
                            <div className="flex gap-2 flex-shrink-0">
                                <Button size="sm" variant="outline" onClick={handleGenerate} className="h-8 text-xs">
                                    <RefreshCw className="w-3 h-3 mr-1" />Redo
                                </Button>
                                <Button size="sm" variant="outline" onClick={handleGenerateQuiz} className="h-8 text-xs border-purple-200 text-purple-700 hover:bg-purple-50">
                                    <Zap className="w-3 h-3 mr-1" />Quiz Me
                                </Button>
                                <Button size="sm" onClick={() => setShowResultModal(true)} className="h-8 text-xs bg-emerald-600 hover:bg-emerald-700">
            ...
            <AIFeedbackModal
                ...
                accentColor="emerald"
                actions={[
                    { label: 'Quiz Me', icon: Zap, onClick: () => { setShowResultModal(false); handleGenerateQuiz(); }, variant: 'outline', className: 'border-purple-200 text-purple-700 hover:bg-purple-50' },
                    { label: 'Save', icon: Save, onClick: () => { setShowResultModal(false); setSaveTitle(concept); setIsSaveDialogOpen(true); }, className: 'bg-emerald-600 hover:bg-emerald-700 text-white' },
                ]}
            />
