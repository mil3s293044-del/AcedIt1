...
| 2b | Landing + onboarding rebuilt as ONE nine-act reel, no reload between them | done |
...
## The landing page and the onboarding are ONE FILM

**"I want it to feel like an experience and almost like a movie."** The two
halves of the only journey that converts anybody were a brochure and a form,
joined by a white flash. The landing page was thirteen alternating bands fading
up as you passed them; it ended in a button that called
`window.location.assign("/onboarding")` — a FULL DOCUMENT RELOAD — into a
six-step wizard with a grey segmented progress bar. Ninety seconds of momentum,
spent on a navigation.

`src/lib/reel.js` is the model and `src/components/reel/` is the machinery.
Nine acts: six that make the case and three that ask the questions, with
**nothing between the sixth and the seventh**. The "Deal me in" button calls
`advance()`. It scrolls. Same table, same hand, and the next thing on screen is
a question instead of a claim.

**EVERY ACT PAYS OUT A CARD.** This is the wizard's own rule — the reason it
converts — extended BACKWARDS into the landing. `cardFor` mints one per act and
returns NULL rather than a placeholder, the same refusal `previewFor` and the
dashboard rail make: a hand holding "your subjects: —" teaches a student in the
first ninety seconds that the objects here are decoration, after which the real
ones do not land either. `reel.test.mjs` asserts EVERY act refuses empty state,
so a branch added later cannot quietly start printing a dash.

**THE HAND IS WHAT MAKES IT ONE FILM.** Not a transition — a THING that
survives the seam. A card dealt by the forgetting act is still in your hand
while the wizard asks your subjects. It is the progress indicator, and it shows
what you LEARNED rather than how many screens are left. It never shrinks:
scrolling back does not take a card away.

**ACT ONE IS THE YEAR QUESTION.** Pick one of three face-down cards, it turns
over carrying the weeks until your exams. There is deliberately **no second
year step** — asking twice is what a form does when its halves were built by
different people. A test asserts no `year` act exists; the subjects act carries
a chip for anybody who scrolled past.

**THE INTERACTION IS NEVER A GATE.** `gate` is false on every scene and a test
holds it. The people most likely to pay are the least willing to be detained by
a puzzle box.

**SCROLL IS THE ESCAPE HATCH, NOT THE ENGINE.** There is no `preventDefault` on
a wheel event anywhere in the tree. The cinema is snap points and
`scrollIntoView`, both of which the browser owns. **The active act is DERIVED
from an IntersectionObserver, never stored** — `goTo` scrolls and lets the
observer report. State-as-truth with an effect scrolling to match is what
produces the classic bug where the page fights you.

**SNAP IS `proximity` UNTIL EVERY ACT IS MEASURED TO FIT.** `mandatory` has one
catastrophic failure: an act taller than the viewport becomes physically
unreachable below its own fold — the container yanks you back, forever. A phone
in landscape, large type set, a long subject list: any of those. So the Reel
shell measures and upgrades, and re-measures on resize and on any act changing
size.

**KEYS ARE NEVER STOLEN FROM AN INPUT.** The back half of this reel is a form.
`typingInto` stands down on a field, a modifier, and anything inside a dialog or
listbox.

### The three questions render the WIZARD'S OWN COMPONENTS

`Step2Subjects`, `Step4Target` and `Step6Signin` are imported straight out of
`pages/Onboarding.jsx` and exported for it. Same code, same answers shape, same
`STORAGE_KEY` (now `lib/onboardingAnswers.js`, extracted so the eager landing
chunk can read it without dragging the lazy page in) — so a student who answers
two questions in the reel and reloads onto /onboarding resumes mid-sentence.

It needed no refactor: `StepShell` already falls back to rendering its footer IN
PLACE when no `ActionsSlot` is provided. The reel provides none.

**Step6Signin must never be forked.** Its error branches were each written after
something actually went wrong in production. A second copy is a second place for
a sign-up bug to live, on the one screen where a bug costs money.

`eyebrow` is overridable because the two flows COUNT DIFFERENTLY — the wizard
has six steps, the reel asks three questions. It printed "The deal · 2 of 4"
directly under "Question 1 of 3": two counters, one screen, disagreeing.

### What only a screenshot caught

Four of these render perfectly, pass lint and the build, and throw nothing.

- **THE FIXED CHROME HAD NO GROUND TO READ.** The nav and the chapter rail are
  `position: fixed`; they float over whichever act is on screen and belong to
  none of them. Hard-coded ink made the nav a cream bar with near-black type
  over three consecutive DARK acts — an opaque grey slab — and the rail was
  `currentColor`, near-black on near-black, completely invisible. Same class as
  `Countdown variant="banner"`, arrived at from the opposite direction. Every
  act declares `ground` now and a test asserts it, because an act added later
  would otherwise inherit whatever the last one happened to be.
- **THE ACTS DID NOT RESERVE THE CHROME'S SPACE.** Content is centred in
  `100svh`, so the eyebrow sat UNDER the nav — centred perfectly inside a box
  64px taller than what was actually available — and the fixed hand rail landed
  on every act's cue, the one control saying what to do next. The reserve
  differs by breakpoint because the rail does.
- **THE HAND WAS ON SCREEN TWICE** on the turn: the big fan AND the rail. The
  wizard's own reveal step already hides its rail for exactly this reason.
- **ON A PHONE THE RAIL COVERED BODY TEXT.** A 90px fixed bar over acts that are
  legitimately taller than a 390×844 screen. It collapses to a count pill there
  — which is what the rail is actually for — and deals out on tap.

Two more the tests caught: the spaced-recall line **SAWS** and an early
assertion demanded it be monotonic, which would have forced a model that never
recovers — an argument against the product; and `−0` printed on a dropped mark,
which reads as a deduction of nothing.

### The curve is the product's own

`RETENTION_K` in `lib/retention` is derived from SM-2's 90% target, and the
forgetting act plots it. The old marketing chart carried
`const K = Math.log(10 / 9)` written out again — the same number, restated. A
marketing chart that has drifted from the product's model is worse than a wrong
chart: it is a promise the app then fails to keep. `reel.test.mjs` scans the
tree, comments stripped first, so a third copy cannot appear.

`ALONE_S = 4.5` is GENEROUS — a fifth of it survives a week, kinder than the
literature. Overstating the decay to make the pitch land is the thing this
refuses. Same for `tutorGap`: tutoring is costed over 40 school weeks and
AcedIt over 52, the asymmetry that flatters us LEAST, because the honest
comparison already wins and an exaggerated one is checkable by anyone with a
parent.

### What was cut, and the tail

Six bands became acts. Four more — the subject marquee, the stats strip, the
hook, the how-it-works — were restatements of what the acts now demonstrate, and
a demonstration followed by a paragraph repeating it is the paragraph admitting
the demonstration did not land. `DealtHand`, `StepCards`, `EvidenceSplit`,
`MarkedAnswer`, `ForgettingCurve`, `PriceAnchor` and `MarkedWord` are DELETED —
all seven had zero importers and only comment references.

What survives is `ReelTail`: pricing in full, the objections, the footer.
Deliberately **un-snapped**, because giving reference material snap points makes
LEAVING the film feel like being detained by it. `CardStorm` is untouched — it
is the title sequence, and it is mounted above the auth gate on purpose.

**The tier is a BUDGET, not a feature flag** (`deviceTier`). Read once on mount,
never on resize: a narrow window is not a slow machine, and re-tiering on resize
makes a page visibly drop its animations when devtools opens. An UNKNOWN machine
counts as capable — `deviceMemory` is absent on Safari, and guessing weak there
strips the film from every iPhone. Every tier gets a designed reduction, not an
empty stage.

### Key files

- `src/lib/reel.js` + `reel.test.mjs` — the manifest, the hand, what each act
  pays out, the curve, the tutor arithmetic, the device tier
- `src/components/reel/Reel.jsx` — the snap measurement, the keys, the rails
- `src/components/reel/ReelContext.jsx` — active act derived from the observer
- `src/components/reel/Act.jsx` — one act, and the chrome reserve
- `src/components/reel/HandRail.jsx` — the object that survives the seam
- `src/components/reel/acts/` — the six scenes and the three step wrappers
- `src/lib/onboardingAnswers.js` — one record, shared by both routes
